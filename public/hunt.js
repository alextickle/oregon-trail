...
function clearAnimalTimer() {
	...
	document.getElementById('results').innerHTML =
		'You killed ' + state.kills + ' animals.';
...
function bulletAdvances() {
	if (state.currentBullet.row >= 0) {
		if (state.currentBullet.row == 0) {
			...
			if (
				state.grid[state.currentBullet.row - 1][state.currentBullet.column] ==
				'a'
			) {
				state.grid[state.currentBullet.row - 1][state.currentBullet.column] =
					'd';
				console.log('kill!');
				state.clearRows.splice(
					state.clearRows.indexOf(state.currentBullet.row - 1),
					1
				);
				state.kills++;
				state.grid[state.currentBullet.row][state.currentBullet.column] = '';
